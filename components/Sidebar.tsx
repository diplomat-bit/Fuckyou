...
import { NAV_SECTORS, AI_MODULES } from '../constants';
import { ShieldCheck, LogOut, Network, ChevronRight, FileCode } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, openTab, isOpen, setIsOpen }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div className={`flex flex-col w-[300px] bg-[#020617]/95 backdrop-blur-3xl border-r border-white/5 fixed lg:relative inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-out will-change-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="space-y-0.5">
                                {AI_MODULES.map((url, index) => (
                                    <button
                                        key={index}
                                        onClick={() => setActiveView(`IFRAME_VIEW:${url}`)}
                                        className="w-full flex items-center px-4 py-2.5 transition-all duration-300 rounded-[1rem] group relative text-gray-500 hover:text-gray-200 hover:bg-white/5"
                                    >
                                        <FileCode size={18} className="text-cyan-400 group-hover:scale-110 transition-transform" />
                                        <span className="mx-3 font-black text-[10px] uppercase tracking-[0.15em] truncate">
                                            {url.split('//')[1].split('.')[0].replace('admin08077-', '')}
                                        </span>
                                    </button>
                                ))}
                            </div>
