...
As part of the broader Oko-main architecture, `InvestmentsView` now interfaces directly with:
- `api/crypto-strategy.ts` and `api/tqqq-strategy.ts` for advanced algorithmic asset management.
- `services/AlpacaTradingService.ts` for real-time execution of portfolio rebalancing.
- `tables/accounts.ts` and `tables/transactions.ts` for persistent ledger synchronization.
- `trillionaire-status/` modules for deep-dive market cap and ESG impact analysis.

This integration ensures that the Celestial Observatory is not merely a visual dashboard, but a functional command center connected to the global financial fabric.