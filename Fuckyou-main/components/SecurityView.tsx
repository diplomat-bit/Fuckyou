...
const SecurityView: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'devices' | 'data_sharing' | 'attestation' | 'orchestrator'>('overview');
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex space-x-4 border-b border-gray-700 pb-1 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('orchestrator')} className={`pb-2 px-1 text-sm font-medium ${activeTab === 'orchestrator' ? 'text-cyan-400 border-b-2 border-cyan-400' : 'text-gray-400 hover:text-gray-200'}`}>Orchestrator</button>
            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-col gap-2">
                                <PlaidLinkButton onSuccess={async (accessToken, meta) => {
                                    try {
                                        const accountsResponse = await axios.post('/api/v1/plaid/accounts', { access_token: accessToken });
                                        const transactionsResponse = await axios.post('/api/v1/plaid/transactions', { access_token: accessToken, start_date: '2024-01-01', end_date: '2024-12-31' });
            ...
            {activeTab === 'attestation' && <HoKTokenMint />}
            {activeTab === 'orchestrator' && <SecurityOrchestratorView />}
