...
import { ShoppingCart, Sparkles, Tag, ArrowRight, Loader2, Search, Building2, Landmark, ShieldCheck, Globe } from 'lucide-react';
...
interface AgoraProduct {
  ...
  description: string;
  aiReason: string;
  type: 'commercial' | 'sovereign' | 'real-estate' | 'tax-lien';
}
...
const MarketplaceView: React.FC = () => {
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    setError(null);
    try {
      // Summarize transactions for the AI context
      const contextSummary = transactions
        .slice(0, 10)
        .map(t => `${t.metadata?.merchantName || 'Unknown'}: ${t.amount}`)
        .join(', ');
      
      // FIX: Handle potential type mismatch where data might be an array or an object
  ...
  }, [fetchRecommendations]); // Added fetchRecommendations to dependencies

  const getIconForType = (type: string) => {
    switch(type) {
      case 'sovereign': return <Landmark className="w-4 h-4" />;
      case 'real-estate': return <Building2 className="w-4 h-4" />;
      case 'tax-lien': return <ShieldCheck className="w-4 h-4" />;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['all', 'commercial', 'sovereign', 'real-estate', 'tax-lien'].map((filter) => (
          <button 
            key={filter}
            onClick={() => setActiveFilter(filter)}
            className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border transition-all ${activeFilter === filter ? 'bg-cyan-500/20 border-cyan-500 text-cyan-400' : 'bg-gray-900 border-gray-800 text-gray-500 hover:border-gray-600'}`}
          >
            {filter}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.filter(p => activeFilter === 'all' || p.type === activeFilter).map((product) => (
            <Card key={product.id} className="group hover:border-cyan-500/50 transition-all duration-500">
               <div className="aspect-video bg-gray-800 rounded-xl mb-4 overflow-hidden relative flex items-center justify-center">
                  ...
                  <div className="absolute top-4 right-4 bg-gray-950/80 px-3 py-1 rounded-full border border-gray-800 text-[10px] font-black text-cyan-400 uppercase flex items-center gap-2">
                    {getIconForType(product.type)} {product.category}
                  </div>
               </div>
